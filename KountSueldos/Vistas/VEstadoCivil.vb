Public Class VEstadoCivil


    Dim ADEstadoCivil As ADEstadoCivil = New ADEstadoCivil
    Dim validar As Validar = New Validar



    Private Sub VEstadoCivil_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        MConfiguracionF.TamañoForm(Me)
        dgvmanual(dgvestadocivil)
        MConfiguracionF.configDGV(dgvestadocivil)
        Mostrardgv()
        indice()
    ...
    Sub Mostrardgv()
        dgvmanual(dgvestadocivil)
        ADEstadoCivil.MostrarEstado_Civil(dgvestadocivil)
    End Sub
    Sub dgvmanual(dgv As DataGridView)
        ...
        dgv.Columns(5).DataPropertyName = "estadobaja"
    ...
    Sub Eliminar()
        Try
            If MsgBox("Seguro desea Eliminar este Registro?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmar") = MsgBoxResult.Yes Then
                ...
                Dim ide = dgvestadocivil.CurrentRow().Cells(0).Value
                ADEstadoCivil.DarBaja(ide)
                Mostrardgv()
    ...
    Sub indice()
        If ADEstadoCivil.index() = 0 Then
            txtcod.Text = 1
        Else
            txtcod.Text = ADEstadoCivil.index()
        End If
    End Sub
    ...
    Sub Agregar()
        ...
        Else
            ...
            If (ADEstadoCivil.Existe(txtdescripcion.Text) = False) Then

                If MsgBox("Seguro agregar este registro?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmar") = MsgBoxResult.Yes Then

                    ADEstadoCivil.AgregarEstado_Civil(New Estado_Civil() With
                                    {
    ...
    Sub Editar()
        ...
        Try
            If dgvestadocivil.RowCount > 0 Then
                txtcod.Text = CStr(dgvestadocivil.CurrentRow.Cells(0).Value)
                txtdescripcion.Text = dgvestadocivil.CurrentRow.Cells(1).Value
            Else
    ...
    Function ExisteReg() As Boolean
        ...
        Try
            If txtcod.Text.Trim <> "" Or txtcod.Text.Trim <> 0 Then
                ...
                If ADEstadoCivil.ExisteID(id) = True Then
                    res = True
                    ' MsgBox("Existe")
                Else
                    'MsgBox("no Existe")
                ...
                Else
                    If ExisteReg() = True Then
                        ...
                        If MsgBox("Seguro desea dar de modificar este Registro?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Confirmar") = MsgBoxResult.Yes Then
                            ...
                            Dim id As Integer = dgvestadocivil.CurrentRow.Cells(0).Value
                            Dim bas As Decimal = 0

                            ADEstadoCivil.ModificarEstado_Civil(txtcod.Text, txtdescripcion.Text)
    ...
    Sub Refrescar()
        dgvestadocivil.DataSource = Nothing
        Mostrardgv()
        limpiar()
        indice()
    End Sub

    Private Sub dgvestadocivil_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvestadocivil.CellContentClick
        Try
            If validar.DatagridVacio(dgvestadocivil) = False Then
    ...
    Private Sub txtfiltro_TextChanged(sender As Object, e As EventArgs) Handles txtfiltro.TextChanged
        ADEstadoCivil.Filtra(txtfiltro, dgvestadocivil)
    End Sub
End Class